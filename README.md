```
██╗  ██╗ █████╗ ███╗   ███╗██╗██████╗ 
██║  ██║██╔══██╗████╗ ████║██║██╔══██╗
███████║███████║██╔████╔██║██║██║  ██║
██╔══██║██╔══██║██║╚██╔╝██║██║██║  ██║
██║  ██║██║  ██║██║ ╚═╝ ██║██║██████╔╝
╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝╚═╝╚═════╝
```
**Hamid Shafiq**  
Cybersecurity Student · Ethical Hacking · OSINT · Offensive Security  

[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=flat&logo=githubpages&logoColor=white)](https://hamidshafiq1.github.io)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/hamidshafiq/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/HamidShafiq1)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:thehamidfr@gmail.com)

---

## 👾 About Me

> *"Understand the system. Break the system. Secure the system."*

- 🎓 BS Cybersecurity student @ University of Haripur, KPK — Semester 4 | Roll No. DHC-40
- 🔐 Focused on Ethical Hacking, Offensive Security, OSINT, and Penetration Testing
- 🏴 CTF Competitor — Team **0UTL4W3D**
- 📍 Based in Wah Cantt, Punjab, Pakistan
- 🌱 Currently exploring: Linux internals, exploit development, and web security
- 💼 Open to internships in Penetration Testing, Secure Development & Cybersecurity Research

---

## 🛠️ Focus Areas

| Domain | Tools & Skills |
|--------|----------------|
| 🔴 Ethical Hacking / Offensive Security | Nmap, Metasploit, Burp Suite |
| 🔍 OSINT | theHarvester, Maltego, Shodan, Google Dorks |
| 🌐 Network Security | Wireshark, Cisco Packet Tracer, TCP/IP Analysis |
| 🐧 Linux Development | Bash Scripting, C++17, Python |
| 🗄️ Database Security | MySQL, DBMS Concepts, SQL Injection Analysis |
| 🏴 CTF / Competitive Security | picoCTF, Forensics, Web Exploitation, Cryptography |

---

## 🚀 Featured Projects

### 🌐 Personal Cybersecurity Portfolio
Live portfolio site built with pure HTML, CSS, and vanilla JS — no frameworks, no build tools, no dependencies. Showcases projects, certifications, and skills. Deployed via GitHub Pages.  
`HTML` `CSS` `JavaScript` `GitHub Pages`  
[🔗 hamidshafiq1.github.io](https://hamidshafiq1.github.io)

---

### 🔢 Scientific Calculator (C++17)
Multi-file C++17 scientific calculator with live GitHub Pages web interface. Covers modular design, OOP principles, and clean build architecture.  
`C++17` `OOP` `GitHub Pages` `Multi-file Architecture`  
[🔗 hamidshafiq1/scientific-calculator-cpp](https://github.com/hamidshafiq1/scientific-calculator-cpp)

---

### 🏴 0UTL4W3D — CTF Team
Competitive CTF team covering Forensics, Cryptography, Web Exploitation, OSINT, and Reverse Engineering. Built around structured skill development and challenge practice via picoCTF and similar platforms.  
`CTF` `Forensics` `Cryptography` `OSINT` `Web Exploitation` `Reverse Engineering`

---

### 🗄️ MySQL Lab Series — Database Security Fundamentals
Hands-on MySQL labs covering DDL/DML operations, table design, INSERT/UPDATE/DELETE, AUTO_INCREMENT, and safe update mode configuration.  
`MySQL` `DBMS` `DDL/DML` `Database Security`

---

## 📜 Certifications

| Certificate | Issuer |
|-------------|--------|
| Fundamentals of Game Hacking Development | — |
| Foundations of Cybersecurity | Google |
| Play It Safe: Manage Security Risks | Google |
| Introduction to Linux (LFS101) | Linux Foundation |
| Introduction to Cybersecurity (ICYS) | Cisco |

---

## 📊 GitHub Stats

![Hamid's GitHub Stats](https://github-readme-stats.vercel.app/api?username=hamidshafiq1&show_icons=true&theme=radical&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=hamidshafiq1&layout=compact&theme=radical&hide_border=true)

---

*Always learning. Always breaking. Always securing.*
